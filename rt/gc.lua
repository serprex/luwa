gccollect = func(function(f)
	local freetip, livetip, sz, n, m = f:i32(), f:i32(), f:i32(), f:i32(), f:i32()
	f:loadg(markbit)
	f:eqz()
	f:storeg(markbit)

	-- Phase0 marking phase
	f:loadg(onil)
	f:call(gcmark)
	f:loadg(ofalse)
	f:call(gcmark)
	f:loadg(otrue)
	f:call(gcmark)
	f:loadg(otmp)
	f:call(gcmark)
	f:loadg(otmpstack)
	f:call(gcmark)

	f:call(igcmark)

	-- Phase1 set reloc pointers
	f:loadg(markbit)
	f:store(freetip)

	f:loop(function(loop)
		f:load(livetip)
		f:call(sizeof)
		f:store(sz)

		f:load(livetip)
		f:i32load()
		f:loadg(markbit)
		f:eq()
		f:iff(function()
			f:load(livetip)
			f:load(freetip)
			f:i32store()

			f:load(freetip)
			f:load(sz)
			f:i32add()
			f:store(freetip)
		end)

		f:load(livetip)
		f:load(sz)
		f:add()
		f:tee(livetip)
		f:loadg(heaptip)
		f:ne()
		f:brif(loop)
	end)

	-- Phase2 fix reloc pointers
	f:loadg(otmp)
	f:i32load()
	f:i32(-8)
	f:band()
	f:storeg(otmp)

	f:loadg(otmpstack)
	f:i32load()
	f:i32(-8)
	f:band()
	f:storeg(otmpstack)

	f:i32(0)
	f:store(livetip)
	f:loop(function()
		f:load(livetip)
		f:call(sizeof) 
		f:store(sz)

		f:load(livetip)
		f:i32load()
		f:i32(1)
		f:band()
		f:loadg(markbit)
		f:eq()
		f:iff(function(terminal)
			f:block(function(vec)
			f:block(function(table)
				f:load(livetip)
				f:i32load8u(4)
				f:brtable(terminal, terminal, terminal, terminal, table, terminal, vec)
			end) -- @table
			f:load(livetip)
			f:load(livetip)
			f:i32load(13)
			f:i32load()
			f:i32(-8)
			f:band()
			f:i32store(13)
			f:load(livetip)
			f:load(livetip)
			f:i32load(17)
			f:i32load()
			f:i32(-8)
			f:band()
			f:i32store(17)
			f:load(livetip)
			f:load(livetip)
			f:i32load(21)
			f:i32load()
			f:i32(-8)
			f:band()
			f:i32store(21)
			end) -- @vec
			f:load(livetip)
			f:tee(n)
			f:load(livetip)
			f:i32load(5)
			f:add()
			f:store(m)
			f:loop(function(loop)
				f:load(n)
				f:load(m)
				f:eq()
				f:brif(1)

				f:load(n)
				f:load(n)
				f:i32load(9)
				f:i32load()
				f:i32(-8)
				f:band()
				f:i32store(9)

				f:load(n)
				f:i32(4)
				f:add()
				f:store(n)

				f:br(loop)
			end)
		end)

		f:load(livetip)
		f:load(sz)
		f:add()
		f:tee(livetip)
		f:loadg(heaptip)
		f:ne()
		f:brif(0)
	end)

	f:call(igcfix)

	-- Phase3 move it
	f:i32(0)
	f:store(livetip)
	f:block(function(f)
		f:block(function(foundshift)
		f:loop(function(loop)
			f:load(livetip)
			f:call(sizeof)
			f:store(sz)

			f:load(livetip)
			f:i32load()
			f:i32(1)
			f:band()
			loadg(markbit)
			f:eq()
			f:iff(function()
				f:load(livetip)
				f:load(livetip)
				f:i32load()
				f:i32(-8)
				f:band()
				f:tee(n)
				f:ne()
				f:brif(foundshift)
			end)

			f:load(livetip)
			f:load(sz)
			f:add()
			f:tee(livetip)
			f:loadg(heaptip)
			f:eq()
			f:brtable(loop, f)
		end)
		end)

		f:load(n)
		f:load(livetip)
		f:load(sz)
		f:call(memcpy8)

		f:load(livetip)
		f:load(sz)
		f:add()
		f:tee(livetip)
		f:loadg(heaptip)
		f:ne()
		f:iff(function(blif)
			f:loop(function(loop)
				f:load(livetip)
				f:call(sizeof)
				f:store(sz)

				f:load(livetip)
				f:i32load()
				f:i32(1)
				f:band()
				f:loadg(markbit)
				f:eq()
				f:iff(function()
					f:load(livetip)
					f:i32load()
					f:i32(-8)
					f:band()
					f:load(livetip)
					f:load(sz)
					f:call(memcpy8)
				end)

				f:load(livetip)
				f:load(sz)
				f:add()
				f:tee(livetip)
				f:loadg(heaptip)
				f:eq()
				f:brtable(loop, blif)
			end)
		end)

		f:load(freetip)
		f:i32(-8)
		f:band()
		f:storeg(heaptip)
	end)
end)

gcmark = export(func(function(f)
	local o = f:params(i32)
	local m = f:i32()
	-- check liveness bit
	f:load(o)
	f:i32load()
	f:i32(1)
	f:band()
	f:loadg(bitmark)
	f:ne()
	f:iff(function()
		f:load(o)
		f:loadg(markbit)
		f:i32store()

		f:block(function(vec)
		f:block(function(table)
			f:load(o)
			f:i32load8u(4)
			f:br_table(gcmark, gcmark, gcmark, gcmark, table, gcmark, vec)
		end) -- @table
		f:load(o)
		f:i32load(13)
		f:call(gcmark)
		f:load(o)
		f:i32load(17)
		f:call(gcmark)
		f:load(o)
		f:i32load(21)
		f:call(gcmark)
		f:ret()
		end) -- @vec
		f:load(o)
		f:load(o)
		f:i32load(5)
		f:add()
		f:store(m)
		f:loop(function(loop)
			f:load(o)
			f:load(m)
			f:eq()
			f:brif(f)

			f:load(o)
			f:i32load(9)
			f:call(gcmark)

			f:load(o)
			f:i32(4)
			f:add()
			f:store(o)

			f:br(loop)
		end)
	end)
end))

tmppush = export(func(function(f)
	local o = f:params(i32)
	local n, p = f:i32(), f:i32()
	f:loadg(otmpstacklen)
	f:loadg(otmpstack)
	f:i32load(5)
	f:eq()
	f:iff(function()
		f:load(o)
		f:storeg(otmp)

		f:loadg(otmpstacklen)
		f:loadg(otmpstacklen)
		f:add()
		f:tee(n)
		f:call(newvec)
		f:tee(p)

		f:load(p)
		f:loadg(otmpstack)
		f:loadg(otmpstacklen)
		f:i32(9)
		f:i32add()
		f:call(memcpy8)

		f:storeg(otmpstack)

		f:load(p)
		f:load(n)
		f:i32store(5)

		f:loadg(otmp)
		f:store(o)
	end)
	f:loadg(otmpstack)
	f:loadg(otmpstacklen)
	f:add()
	f:load(o)
	f:i32store(9)

	f:loadg(otmpstacklen)
	f:i32(4)
	f:add()
	f:storeg(otmpstacklen)
end))

tmppop = export(func(function()
	f:loadg(otmpstacklen)
	f:i32(4)
	f:sub()
	f:storeg(otmpstacklen)

	f:loadg(otmpstack)
	f:loadg(otmpstacklen)
	f:add()
	f:loadg(onil)
	f:i32store(9)
end))

nthtmp = export(func(i32, function(f)
	local i = f:params(i32)
	f:loadg(otmpstack)
	f:loadg(otmpstacklen)
	f:load(i)
	f:i32(2)
	f:shl()
	f:sub()
	f:add()
	f:i32load(9)
end))

setnthtmp = export(func(function(f)
	local nv, i = f:params()
	f:loadg(otmpstack)
	f:loadg(otmpstacklen)
	f:load(i)
	f:i32(2)
	f:shl()
	f:sub()
	f:add()
	f:load(nv)
	f:i32store(9)
end)
